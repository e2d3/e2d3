...
module.exports = function (options) {
  ...
  app.get('/api/categories/develop', api('', options));
  app.get('/api/categories/delegate', api('delegate:', options));

  app.use('/files/github/e2d3/e2d3-contrib/contents', express.static(options.contrib));
  ...
  if (options.http) {
    setupServer(http.createServer(app)).listen(options.http, function () {
      console.log(colors.green('[E2D3] Webserver started at http://locahost:' + options.http));
      if (options.livereload === -1) {
  ...
  if (options.https) {
    ...
    setupServer(https.createServer(credentials, app)).listen(options.https, function () {
      console.log(colors.green('[E2D3] Webserver(SSL) started at https://localhost:' + options.https));
